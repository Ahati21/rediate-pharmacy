...
export default function Login() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Generate a capitalized name from the email prefix (e.g., abebe@... -> Abebe)
    const namePrefix = emailInput.split('@')[0];
    const generatedName = namePrefix ? namePrefix.charAt(0).toUpperCase() + namePrefix.slice(1).replace(/\./g, ' ') : '';
    
    login(selectedRole, generatedName);
    if (selectedRole === 'customer') navigate('/');
    else if (selectedRole === 'pharmacist') navigate('/pharmacist/queue');
