...
export default function CustomerHome() {
  ...
  const approvedPrescription = prescriptions.find(p => p.status === 'approved');
    ...
    <div className="max-w-5xl mx-auto px-6 py-8 pb-32 bg-[#F7F9FA] dark:bg-slate-900 min-h-screen font-sans">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-start justify-between gap-6">
        <div className="flex-1">
          <p className="text-[#6B7280] dark:text-slate-400 text-[10px] font-bold tracking-[0.15em] uppercase mb-2">Welcome back, {user?.name?.split(' ')[0] || 'Patient'}</p>
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-[#F2F4F5] dark:bg-slate-800 rounded-[12px] py-4 pl-12 pr-4 text-[14px] text-[#111827] dark:text-white outline-none border-none placeholder-[#9CA3AF] shadow-sm cursor-pointer"
            />
          </div>
        </div>
      </div>
      ...
      {approvedPrescription && (
        <div className="bg-[#EAF3EE] dark:bg-emerald-900/20 border border-[#CDE5D8] dark:border-emerald-900/50 rounded-[24px] p-6 flex flex-col items-center text-center mb-8 shadow-sm">
          ...
          <p className="text-[13px] text-[#111827] dark:text-emerald-400 font-medium leading-relaxed mb-6">
            Your prescription for <span className="font-bold">{approvedPrescription.fileName || 'your medication'}</span> has been <span className="font-bold">approved</span> by the pharmacist! Review details and complete your checkout to begin delivery.
          </p>
          <button 
      ...
      <div className="mb-12">
        ...
        <div className="flex overflow-x-auto gap-4 pb-4 -mx-6 px-6 no-scrollbar">
          <div onClick={() => navigate('/search')} className="min-w-[110px] bg-white dark:bg-slate-800 rounded-[16px] p-5 flex flex-col items-center text-center shadow-sm border border-gray-100 dark:border-slate-700 cursor-pointer hover:shadow-md transition-all">
      ...
      <div className="bg-[#F2F4F5] dark:bg-slate-800/50 rounded-[32px] p-8 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          {prescriptions.length === 0 ? (
            <div className="bg-white/40 dark:bg-slate-800/40 rounded-[16px] p-6 text-center border border-dashed border-gray-300 dark:border-slate-700">
               <p className="text-[12px] text-gray-500 font-medium">No prescriptions uploaded yet.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {prescriptions.slice(0, 2).map(p => (
                <div key={p.id} className="bg-white dark:bg-slate-800 rounded-[16px] p-4 flex items-center justify-between shadow-sm border border-white/60 dark:border-slate-700">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-[13px] font-bold text-[#111827] dark:text-white mb-0.5 truncate max-w-[150px]">{p.fileName}</p>
                      <p className="text-[10px] text-[#6B7280] dark:text-slate-400">Status: <span className="font-bold uppercase">{p.status}</span> • {new Date(p.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => navigate('/tracking')}
                    className="text-[#004A8F] dark:text-blue-400 text-[10px] font-black uppercase tracking-widest hover:underline px-2"
                  >
                    Track
                  </button>
                </div>
