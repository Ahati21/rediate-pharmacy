...
export default function ScriptUpload() {
  ...
  const { setPrescriptionDraft } = useOrder();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (selectedFile) {
        fileContent = await readFileAsDataUrl(selectedFile);
        fileType = selectedFile.type;
      }

      const response = await api.post<ApiItemResponse<{ _id: string; doctorName: string; notes: string; fileName?: string }>>('/prescriptions', {
        patientName: userName || user?.name || 'Customer',
      ...
      setError(submitError instanceof Error ? submitError.message : 'Unable to submit prescription');
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-[#F8FAFB] min-h-screen font-sans pb-32">
      ...
      <div className="px-6 pt-8 pb-4">
        ...
        <div onClick={handleBrowseClick} className="bg-white rounded-[32px] p-8 border-2 border-dashed border-gray-200 flex flex-col items-center text-center mb-10 group hover:border-[#004A8F] transition-all cursor-pointer">
          <input
            type="file"
            ref={fileInputRef}
            className="hidden"
          ...
          <p className="text-[12px] font-medium text-[#6B7280] mb-2">JPG, PNG or PDF (Max 10MB)</p>
          {selectedFileName && (
            <p className="text-[12px] font-black text-[#004A8F] mb-4">{selectedFileName}</p>
          )}
          <button
            onClick={(e) => {
              e.stopPropagation();
