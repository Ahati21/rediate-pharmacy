    ...
    <div className="max-w-4xl mx-auto bg-[#F8FAFB] min-h-screen font-sans pb-32">
      ...
      <div className="px-6 py-10 flex flex-col items-center text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-10 w-full max-w-sm">
          <button onClick={() => navigate('/tracking')} className="flex-1 bg-[#004A8F] text-white font-black py-5 rounded-[16px] flex items-center justify-center gap-3 shadow-xl shadow-[#004A8F]/20 hover:bg-[#003870] transition-all">
            <span className="material-symbols-outlined text-[20px]">local_shipping</span>
            Track Order Status
          </button>
          <button onClick={() => navigate('/')} className="flex-1 bg-[#F3F4F6] text-[#4B5563] font-black py-5 rounded-[16px] hover:bg-gray-200 transition-all">
            Back to Home
          </button>
        </div>
      </div>
      ...
      <div className="px-6 space-y-4">
        <div className="bg-[#004A8F] rounded-[20px] p-6 text-white flex justify-between items-center shadow-lg">
          <div>
            ...
            <h3 className="text-[24px] font-black">{orderDraft?.orderNumber || 'Pending'}</h3>
          </div>
        ...
        <div className="bg-[#F3F4F6]/50 rounded-[32px] p-8 border border-gray-100">
          <div className="flex items-center justify-between mb-8">
            ...
            <span className="bg-white text-[#6B7280] text-[10px] font-black px-3 py-1 rounded-full shadow-sm border border-gray-100">
              {orderDraft?.items.length || 0} Items in package
            </span>
          </div>

          <div className="space-y-6 mb-10">
            {orderDraft?.items.map((item, index) => (
              <div key={`${item.name}-${index}`} className="bg-white rounded-[24px] p-5 flex gap-4 shadow-sm">
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <h4 className="text-[15px] font-black text-[#111827]">{item.name}</h4>
                    <p className="text-[15px] font-black text-[#111827]">{(item.unitPrice * item.quantity).toFixed(2)} <span className="text-[11px] text-[#6B7280]">ETB</span></p>
                  </div>
                  <p className="text-[11px] font-medium text-[#6B7280]">Prepared from live catalog records</p>
                  <p className="text-[11px] font-black text-[#6B7280] mt-1">Qty: {item.quantity.toString().padStart(2, '0')}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-6 pt-8 border-t border-gray-200">
            ...
            <div className="flex justify-between items-end pt-4">
              ...
              <div className="flex items-baseline gap-1">
                <span className="text-[36px] font-black text-[#004A8F]">{orderDraft?.totalAmount.toFixed(2) || '0.00'}</span>
